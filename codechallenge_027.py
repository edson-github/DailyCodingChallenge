...
def main():
	...
	if len(itinerary) > 0:
		print(
			f"Test1:\nGiven the starting airport is {start} and available flights [{', '.join(str(f) for f in flights)}]\nPossible itinerary is [{f'{start}->' + '->'.join(str(i[1]) for i in itinerary)}]"
		)
		print()
